...
export type OrganizerEventField = keyof OrganizerEventInput
...
export function validateOrganizerEvent(input: OrganizerEventInput) {
  ...
  if (!input.venue.trim()) errors.venue = 'Nhập địa điểm tổ chức.'
  if (!input.city.trim()) errors.city = 'Nhập tỉnh/thành phố.'
